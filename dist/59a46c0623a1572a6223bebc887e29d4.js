...
ace.define("ace/ext/emmet", [
    ...
    var AceEmmetEditor = /** @class */ function() {
        ...
        AceEmmetEditor.prototype.getCurrentLineRange = function() {
            var _$ace = this.ace;
            var row = _$ace.getCursorPosition().row;
            var lineLength = _$ace.session.getLine(row).length;
            var index = _$ace.session.doc.positionToIndex({
                row: row,
                column: 0
            });
            return {
    ...
    exports1.commands = new HashHandler();
    exports1.runEmmetCommand = function runEmmetCommand(editor) {
        ...
        try {
            ...
            if (!emmet) {
                var loading = exports1.load(runEmmetCommand.bind(this, editor));
    ...
    for(var command in keymap){
        exports1.commands.addCommand({
            name: "emmet:" + command,
            action: command,
            bindKey: keymap[command],
            exec: exports1.runEmmetCommand,
            multiSelectAction: "forEach"
        });
    }
    exports1.updateCommands = function(editor, enabled) {
        if (enabled) {
            editor.keyBinding.addKeyboardHandler(exports1.commands);
        } else {
            editor.keyBinding.removeKeyboardHandler(exports1.commands);
        }
    };
    exports1.isSupportedMode = function(mode) {
        if (!mode) return false;
        if (mode.emmetConfig) return true;
        var id = mode.$id || mode;
        return /css|less|scss|sass|stylus|html|php|twig|ejs|handlebars/.test(id);
    };
    exports1.isAvailable = function(editor, command) {
        if (/(evaluate_math_expression|expand_abbreviation)$/.test(command)) return true;
        var mode = editor.session.$mode;
        var isSupported = exports1.isSupportedMode(mode);
    ...
    var onChangeMode = function onChangeMode(e, target) {
        var editor = target;
        if (!editor) return;
        var enabled = exports1.isSupportedMode(editor.session.$mode);
        if (e.enableEmmet === false) enabled = false;
        if (enabled) exports1.load();
        exports1.updateCommands(editor, enabled);
    };
    exports1.load = function(cb) {
    ...
    exports1.AceEmmetEditor = AceEmmetEditor;
